...
class ExportRecentChangesActionTest {
    
    private lateinit var action: ExportRecentChangesAction
    ...
    @BeforeEach
    fun setUp() {
        action = ExportRecentChangesAction()
    ...
    fun `test action presentation text and description`() {
        assert(action.templatePresentation.text == "Recent Changes")
        assert(action.templatePresentation.description == "Export files modified in the last 24 hours")
    }
    ...
    @Test
    fun `test update enables action when project exists regardless of file selection`() {
        // Given
        every { event.project } returns project
    ...
    @Test
    fun `test update enables action when project exists with empty file array`() {
        // Given
        every { event.project } returns project
