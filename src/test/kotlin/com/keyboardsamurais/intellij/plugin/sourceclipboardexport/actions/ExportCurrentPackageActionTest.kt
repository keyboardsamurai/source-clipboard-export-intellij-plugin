...
class ExportCurrentPackageActionTest {
    
    private lateinit var action: ExportCurrentPackageAction
    ...
    @BeforeEach
    fun setUp() {
        action = ExportCurrentPackageAction()
    ...
    fun `test action presentation text and description`() {
        assert(action.templatePresentation.text == "Include Package Files")
        assert(action.templatePresentation.description == "Export all files in the same package")
    }
