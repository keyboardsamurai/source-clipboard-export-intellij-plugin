...
class RepositorySummary(
    ...
    private fun formatXml(totalChars: Int, totalTokens: Int, top10Files: List<FileStats>): String {
        ...
        sb.appendLine("  <repository-info>")
        sb.appendLine("    <repository-url>${escapeXml(getRepositoryUrl())}</repository-url>")
        sb.appendLine("    <generated-at>${escapeXml(getCurrentDateTime())}</generated-at>")
        ...
        top10Files.forEach { stats ->
            sb.appendLine("      <file>")
            sb.appendLine("        <path>${escapeXml(stats.path)}</path>")
